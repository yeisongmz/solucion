...
if ($q<>'')
{
 ...
 if(count($q)>1)
 {
	...
	$sql="select id,sucursales_id,desdefecha,desde_hora,hasta_hora from sucursales_personal where personal_id ='".$id."'  and estado_registro = 'VIGENTE' ";
		...
		$x=0;
		 ...
		 for($a=0;$a<$x;$a++)
		 {
			 $sql="select razon_sucursal,cliente_id from sucursales where id ='".$matr[$a][0]."'  ";
			  ...
			  $sql="select razon from cliente where id ='".$matr[$a][7]."'  ";
	 		 $res=mysql_query($sql,$con);
			  while($fila2=mysql_fetch_array($res))
			  {
				   $matr[$a][6]=$fila2['razon'];
			  }
		 }
 	}
 }
...
echo '<td >'."<a href='#' style='font-size:14px;font-weight:normal;text-decoration:none;color:#0D21F7;font:Arial;font-family:'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, sans-serif;   >".$matr[$a][3]." </a> ".'</td>';	
		...
		echo '<td align="right" >'."<input type='radio' name='radio' value='".$matr[$a][4]."' id='radio_".$matr[$a][4]."' onClick='window.parent.cargar(this.value); ' >".'</td>';
	echo '</tr>';
 } 
	echo '</tbody>';
	echo '</table>';
