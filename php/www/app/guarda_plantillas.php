...
if($ban==0)
{	
						$query2="INSERT INTO plantillas(sucursales_id,nombre)  VALUES('".$ubicacion_id."','".$nombre."')";

						$resultado = mysql_query($query2);
						
						$query2="select id from plantillas where sucursales_id='".$ubicacion_id."' and nombre='".$nombre."' " ;
				$res=mysql_query($query2,$con);
					
						while($query4 = mysql_fetch_array($res) )
						{
							$plantilla_id = $query4['id'];
						}
					
						for ($x = 0; $x < count($equipos)-1; $x++)
		 				{					
		 					$query2="INSERT INTO deta_plantilla(equipos_id,plantillas_id,cantidad,frecuencia,p_unitario,p_total)  VALUES('".$mat[$x]."','".$plantilla_id."','".$cantidad[$x]."','".str_replace(".","",$unitario[$x])."','0','0')";
...
if($ban==0)
{	
		echo '<script type="text/javascript" language="javascript">window.location.replace("panel_plantillas.php");</script>'; 	
}
else
{
