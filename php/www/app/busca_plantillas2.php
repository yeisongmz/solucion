<?php 
	...
	$query2="SELECT equipos.`descrip`,equipos.`id`,equipos.`unidadMedida`,deta_plantilla.`cantidad`,deta_plantilla.`p_total`,deta_plantilla.`p_unitario` FROM equipos,deta_plantilla WHERE deta_plantilla.plantillas_id='".$nombre."' AND deta_plantilla.`equipos_id`=equipos.`id` ORDER BY `descrip` DESC";
...
(SELECT frecuencia FROM deta_plantilla WHERE plantillas_id = r.plantilla_id AND equipos_id=rd.equipos_id) AS frec_dias,
DATE_ADD(r.fecha,INTERVAL (SELECT frecuencia FROM deta_plantilla WHERE plantillas_id = r.plantilla_id AND equipos_id=rd.equipos_id) DAY) AS sig_envio
