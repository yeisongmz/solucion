...
//			}
	...
	$query10="SELECT * FROM liquiregular WHERE liquiregular.desde='".$inicial."' and liquiregular.hasta='".$final."'  order by personal_id  asc" ;
	//echo $query10."<br>";
...
// ******************************************************************************************				
	...
	;	
		...
		//echo count($mat_cabecera).'<br>';
		for($fila3=0;$fila3<count($mat_cabecera);$fila3++)
		{
				$total_bobif=0;
				...
				for($ii=0;$ii<1;$ii++)
				{
					//echo $mat_personal[$fila3][0].' -- '.$fila3.'<br>';
						$pdf->Cell(40,10,"Personal",0,0 , 'L',false);
						$pdf->Cell(90,10,":  ".iconv('UTF-8', 'windows-1252',$mat_personal[$fila3][0]),0,1);
						$pdf->Cell(40,10,html_entity_decode("Fecha de Emision"),0,0 , 'L',false);
						$pdf->Cell(90,10,":  ".$mat_cabecera[0][0],0,1 , 'L',false);
						$pdf->Cell(40,10,"Periodo",0,0 , 'L',false);		
						$pdf->Cell(30,10,":  ".$mat_cabecera[$fila3][1]."  -  ".$mat_cabecera[$fila3][2],0,1 , 'L',false);				
						$pdf->Line(10, 60, 200, 60); // 20mm from each edge
						$pdf->Cell(40,10,"Ingresos",0,1);
						$pdf->SetFillColor(184,180,180);
						$pdf->Cell(100,10,"Concepto",1,0 , 'L',false);
						$pdf->Cell(40,10,"Importe",1,1 , 'R',false);
						$pdf->Cell(100,10,"Sueldo Basico",1,0 , 'L' );
			
						//$pdf->Cell(40,10,number_format($mat_cabecera[$fila3][3]-$aux3,'0',',','.'),1,1 , 'R' );
						$pdf->Cell(40,10,number_format($aux3,'0',',','.'),1,1 , 'R' );
...
//***************************   INFRESOS - BONIFICACION ***********************************				
						...
						for($ii=0;$ii<count($mat_detalle);$ii++)
						{
							...
							if($mat_detalle[$ii][3]==$mat_cabecera[$fila3][5])
							{
								...
								if($mat_detalle[$ii][2]=="HORAS EXTRAS")
								{
									$pdf->Cell(100,10,"HORAS EXTRAS",1,0,false);
								}
								else
								{
									$pdf->Cell(100,10,$mat_detalle[$ii][1],1,0,false);
								}
					
								
								$pdf->Cell(40,10,number_format($mat_detalle[$ii][0],'0',',','.'),1,1 , 'R' );
								$total_bobif=$total_bobif+$mat_detalle[$ii][0];
							}
						}
						$pdf->Cell(100,10,"Total",1,0 , 'L',true);
						//$pdf->Cell(40,10,number_format($mat_cabecera[$fila3][3]+$aux3,'0',',','.'),1,1 , 'R',true);
						$pdf->Cell(40,10,number_format($total_bobif+$aux3,'0',',','.'),1,1 , 'R',true);
						$pdf->Ln();
						
				}		
						
//***************************   EGRESOS   *************************************************
						$pdf->Cell(30,10,"Egresos",0,1);
						$pdf->Cell(100,10,"Concepto",1,0 , 'L',false);
						$pdf->Cell(40,10,"Importe",1,0 , 'R',false);
						$pdf->Ln();
						//echo count($mat_detalle2).'<br>';
						for($i=0;$i<count($mat_detalle2);$i++)
							{
							//echo $mat_detalle2[$i][3].' -- '.$mat_cabecera[$fila3][5].'<br>';
							if($mat_detalle2[$i][3]==$mat_cabecera[$fila3][5])
								{
						$pdf->Cell(100,10,$mat_detalle2[$i][1],1,0);
						$pdf->Cell(40,10,number_format($mat_detalle2[$i][0],'0',',','.'),1,1 , 'R' );
								}
						}
						$pdf->Cell(100,10,"Total",1,0 , 'L',true);
						$pdf->Cell(40,10,number_format($mat_cabecera[$fila3][7],'0',',','.'),1,1 , 'R',true);
						...
						$pdf->Ln(20);
						$pdf->Cell(40,10,"Recibi Conforme:",0,1 , 'L',false);
						for($ii=0;$ii<count($mat_personal);$ii++)
						{
							//echo $mat_personal[$ii][4].'  --  '.$mat_cabecera[$fila3][5].'<br>';
							if($mat_personal[$ii][4]==$mat_cabecera[$fila3][5])
							{
								$pdf->Cell(40,7,iconv('UTF-8', 'windows-1252',$mat_personal[$ii][0]),0,1 , 'L',false);
								$pdf->Cell(40,7,$mat_personal[$ii][3],0,1 , 'L',false);
							}
						}
