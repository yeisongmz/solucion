...
//	echo $query2;
	...
	while($query4 = mysql_fetch_array($res) )
		...
		$query2="select jornalxhora,jornalxmin from personal where id='".$personal_id."' and estado = 1 " ;
	//echo $query2;
	$res=mysql_query($query2,$con);
...
// ******************************************************************************************
	...
	// PARTE DE ASISTENCIAS	
		$query2="select *  from asistencia where personal_id='".$personal_id."' and fecha_asistencia>='".$desde."' and fecha_asistencia<='".$hasta."' and liquiregular_id='".$id[0]."' " ;
//echo $query2.'<br>';
	$res=mysql_query($query2,$con);
	...
	//echo $aux2.'<br>';	
		...
		$total_cobro_diario	=round((($semana_diurna)*$jornalxhora)+(($semana_nocturna)*$jornalxhora*1.3)+$aux+$aux2);
		//echo $total_cobro_diario.'   total_cobro_diario<br>';
// ******************************************************************************************
// ******************************************************************************************
...
//****************************************************************	
$query2="select * from ausencias where liquiregular_id='".$id[0]."' and personal_id='".$personal_id."' and tipo='Injustificada' " ;
	//$res=mysql_query($query2,$con);
	//echo $query2.'<br>';
	while($query4 = mysql_fetch_array($res) )
		{
			$mat_detalle2[$fila][0]=intval($jornalxhora*$query4['cant_horas']);
