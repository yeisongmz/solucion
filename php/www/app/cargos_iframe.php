...
if ($q<>'')
{
 	...
 	$sql="select * from cargos where cargo LIKE '".$q."%'  order by cargo asc";
}
else
{
	
	$sql="select * from cargos order by cargo asc limit 200";	
}
...
echo '</tr>';
 while($fila=mysql_fetch_array($res)){
	...
	echo '<td >'."<a href='#' style='font-size:12px;font-weight:normal;text-decoration:none;color:#0B0B0B;font:Arial;font-family:'Gill Sans', 'Gill Sans MT', 'Myriad Pro', 'DejaVu Sans Condensed', Helvetica, Arial, sans-serif;'  >".$fila['cargo']." </a> ".'</td>';
